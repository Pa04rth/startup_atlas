...
export async function fetchAndCacheLogo(
  ...
  const possibleExtensions = ["png", "jpg", "svg", "ico"];

  // Check whether this logo already exists in R2.
  for (const ext of possibleExtensions) {
    ...
    try {
      ...
      // Doesn't exist, try next extension.
    }
  }
  ...
  try {
    const res = await fetch(
      ...
      {
        headers: {
          "User-Agent": "startup-atlas/0.1 (contact: parthsohaney04@gmail.com)",
        },
        signal: AbortSignal.timeout(8000),
      },
    );
    ...
    // IMPORTANT: store inside the R2 logos/ folder
    const key = `logos/${safeDomain}.${ext}`;
