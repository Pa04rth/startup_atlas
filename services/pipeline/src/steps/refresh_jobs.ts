...
// for a Greenhouse/Lever board (lib/ats.ts) and keeps job_postings in sync:
// upsert whatever's currently listed, expire whatever this brand no longer
// lists (role filled/closed) — never delete, an expired posting just stops
        ...
        await withRetry(() =>
          pool.query(
            `insert into job_postings
               (brand_id, city_id, title, track, seniority, fresher_friendly, apply_url, source_url, posted_at, expires_at)
             values ($1,$2,$3,$4,$5,$6,$7,$8,$9, now() + interval '${POSTING_TTL}')
            ...
            [brand.id, cityId, job.title, job.track, job.seniority, job.fresherFriendly, job.applyUrl, job.sourceUrl, job.postedAt]
          )
        );
        upserted++;
      }
